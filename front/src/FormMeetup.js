...
class FormMeetup extends Component {
  ...
  handleSubmit(e) {
    ...
    if (this.props.match.params.id) {
      ajax
        ...
        .catch(e =>
          alert("Erreur dans la mise à jour, veuillez réessayer plus tard")
        );
    } else {
  ...
  /**
   * Gestion du Formulaire (without input checkbox/radios)
   */
  handleChange(e) {
    this.setState({
  ...
  render() {
    return (
      <form onSubmit={this.handleSubmit}>
        ...
        <div className="form-group">
          <label htmlFor="date_begin">Date du meetup</label>
          <input
            type="date"
            className="form-control"
            name="date_begin"
            id="date_begin"
            placeholder="Ex: 2018-03-21"
